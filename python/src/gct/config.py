...
MCLDUMP_PROG = os.path.join(GCT_HOME, "submodules/local/bin/mcxdump")
MCL_CONVERT_PROG = os.path.join(GCT_HOME, "submodules/local/bin/mcxload")
...
def get_cdc_prog(name, is_directed=None):
    if name in {'k_clique', '2009-cis', '2009-eagle', 'conga-1.0-SNAPSHOT.jar', 'calcJaccards', 'clusterJaccards',
                'topgc-1.0-SNAPSHOT.jar', 'mkidx', '2011-gce', '2011-moses', 'max-clique', '2012-ParCPM','2012-fast-cpm'}:
